...
object DataMapper {
    ...
    fun mapperDataToDomain(input: List<TramoEntity>): List<TramoModel> {
        ...
        input.map {
            val data = TramoModel(uid = it.uid, total = it.total, statusMoney = it.statusMoney,description = it.description, date = it.date)
    ...
    fun mapperDomainToData(input: TramoModel): TramoEntity{
        return TramoEntity(uid = input.uid!!, total = input.total, statusMoney = input.statusMoney, description = input.description, date = input.date)
    }
}